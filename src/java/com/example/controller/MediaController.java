...
public class MediaController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println(fmm.getPictureCount() + " imágenes, " + fmm.getVideoCount() + " vídeos. </p>" );
            for (MediaGroup g : fmm.getGroups()) {
                for (MediaItem item : g.getItems()) {
                    out.println(item.getTitle());
                    out.println("<br>");
                }
            }
            out.println("<p><a href='" + getServletContext().getContextPath() + "/upload.html'>Subir nuevo archivo</a>");
            out.println("</body>");
            out.println("</html>");
        }
    }
